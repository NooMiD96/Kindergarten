...
namespace MyMedicine.Controllers
{
    public partial class ImportExportController
    {
        ...
        private async ValueTask<bool> SeparationDeserialize(string context, int type)
        {
            try
            {
                var symptomList = JsonConvert.DeserializeObject<List<Separation>>(context);
                await _context.ChangeSeparationListAsync(symptomList, type);
            }
            catch
            {
                return false;
